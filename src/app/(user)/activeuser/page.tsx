...
const activeUserList= userList.filter((elm:any)=>elm.is_active === true)
  ...
  return (
    ...
    <div >
        {t && <>
           ...
           <Link href={"/dashboard"} >
           <h1 className='text-center   text-3xl font-[900] w-full'>{`Active User List :- ${activeUserList.length}`}</h1>
           </Link>
           
           </div>
        <div className="pt-36 md:pt-40 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 mx-10  gap-8  mb-10">
            {activeUserList?.map((user:any,index)=>(
